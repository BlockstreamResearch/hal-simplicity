...
pub fn exec<'a>(matches: &clap::ArgMatches<'a>, client: &HalSimplicity) {
	let tx_hex = matches.value_of("tx").expect("tx mandatory").to_string();
	...
	let cmr = matches.value_of("cmr").expect("cmr is mandatory").to_string();
	let control_block = matches.value_of("control-block").map(String::from);
	let genesis_hash = matches.value_of("genesis-hash").map(String::from);
	let secret_key = matches.value_of("secret-key").map(String::from);
	let public_key = matches.value_of("public-key").map(String::from);
	let signature = matches.value_of("signature").map(String::from);
	let input_utxos: Option<Vec<String>> =
		matches.values_of("input-utxo").map(|vals| vals.map(String::from).collect());

	match client.simplicity_sighash(
		tx_hex,
		input_idx,
		cmr,
		control_block,
